...
// ============================================================
const ST = {
  page:   { minHeight:'100vh', background:'#0A1628', fontFamily:'Montserrat,sans-serif', color:'#F0F4FF' } as React.CSSProperties,
...
export default function CajaPage() {
  ...
  // Modal PIN (aparece DESPUÉS de llenar el formulario)
  ...
  async function confirmPin() {
    ...
    setPin('')
    setPinError('')
    if (pendingActionRef.current === 'open') await executeOpen()
    if (pendingActionRef.current === 'close') await executeClose()
      ...
      <div style={ST.modal}>
        <div style={{ textAlign:'center', marginBottom:18 }}>
          ...
          <div style={{ fontSize:11, color:'#8899BB', marginTop:4 }}>
            {pendingActionRef.current === 'open' ? `Abriendo ${registerName}` : 'Cerrando caja'}
          </div>
        </div>
        ...
        <div style={{ display:'flex', justifyContent:'center', gap:10, marginBottom:16 }}>
          {[0,1,2,3,4,5].map(i => (
            <div key={i} style={{ width:14, height:14, borderRadius:'50%', background: i < pin.length ? '#5DE0E6' : 'rgba(93,224,230,.15)', border:'1px solid rgba(93,224,230,.3)', transition:'background .15s' }} />
          ))}
        </div>
        ...
        <input
          ...
          onKeyDown={e => { if (e.key === 'Enter') confirmPin(); if (e.key === 'Escape') setShowPinModal(false) }}
          placeholder="Ingresa tu PIN"
          maxLength={6}
          style={{ ...ST.input, textAlign:'center', fontSize:24, letterSpacing:8, fontWeight:700, marginBottom:pinError ? 8 : 14 }}
        />
        {pinError && (
          <div style={{ background:'rgba(239,68,68,.1)', border:'1px solid rgba(239,68,68,.2)', borderRadius:6, padding:'6px 10px', fontSize:11, color:'#EF4444', marginBottom:12, textAlign:'center' }}>
            {pinError}
          </div>
        )}

        <div style={{ display:'flex', gap:8 }}>
          <button onClick={() => setShowPinModal(false)} style={{ ...ST.btn, flex:1, padding:11, fontSize:12, background:'rgba(255,255,255,.05)', border:'1px solid rgba(255,255,255,.1)', color:'#8899BB' }}>
            Cancelar
          </button>
          <button onClick={confirmPin} disabled={pin.length < 4 || pinLoading} style={{ ...ST.btn, flex:2, padding:11, fontSize:13, background: pin.length >= 4 ? 'linear-gradient(90deg,#004AAD,#5DE0E6)' : 'rgba(0,74,173,.2)', color:'#fff', opacity: pin.length >= 4 ? 1 : .5 }}>
      ...
      <div style={ST.body}>
        ...
        <div style={ST.card}>
          ...
          {[
            ...
            closeResult.transbank_contado > 0 ? ['Transbank / máquina contado', fmt(closeResult.transbank_contado)] : null,
            ['Monto esperado (efectivo)', fmt(closeResult.expected_amount)],
            ['Total contado',             fmt(closeResult.closing_amount)],
          ].filter((item): item is string[] => item !== null).map(([l, v]) => (
