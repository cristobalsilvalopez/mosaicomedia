...
export default function CRMPage() {
  ...
  // Tab activo en lista (list | intelligence)
    ...
    <div style={S.page}>
      ...
      <div style={S.body}>
        ...
        <div style={{ display:'flex', gap:4, background:'#111827', border:'1px solid rgba(93,224,230,.1)', borderRadius:10, padding:4, marginBottom:14, width:'fit-content' }}>
          {([['list','📋 Clientes'],['intelligence','🧠 Inteligencia']] as const).map(([t,l]) => (
        ...
        {activeTab === 'intelligence' && (() => {
          ...
          const vipReady  = customers.filter(c => calcScore(c) >= 70 && c.tier === 'standard' && c.total_purchases > 0)
          const newRecent = customers.filter(c => c.segment === 'new' && (c.days_since_purchase ?? 999) <= 20)
          const allOpps   = customers.flatMap(c => detectOpportunities(c).map(o => ({ ...o, customer: c }))).slice(0, 15)
          ...
          return (
            <div>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:10, marginBottom:16 }}>
                {[
                  { label:'🔥 En riesgo',      count: atRisk.length,    color:'#EF4444', bg:'rgba(239,68,68,.08)',    seg:'at_risk' },
                  { label:'😴 Dormidos',        count: dormant.length,   color:'#F59E0B', bg:'rgba(245,158,11,.08)',  seg:'dormant' },
                  { label:'💎 Potencial VIP',   count: vipReady.length,  color:'#C19E4D', bg:'rgba(193,158,77,.08)',  seg:'' },
                  { label:'🆕 Nuevos activos',  count: newRecent.length, color:'#22C55E', bg:'rgba(34,197,94,.08)',   seg:'new' },
                ].map(k => (
                  <div key={k.label} onClick={() => { if (k.seg) { setFilterSegment(k.seg); setActiveTab('list') } }}
                    style={{ background: k.bg, border:`1px solid ${k.color}30`, borderRadius:10, padding:'12px 14px', cursor: k.seg ? 'pointer' : 'default' }}>
                    <div style={{ fontSize:11, color: k.color, fontWeight:600 }}>{k.label}</div>
                    <div style={{ fontSize:26, fontWeight:800, color:'#F0F4FF', marginTop:4 }}>{k.count}</div>
                    {k.seg && <div style={{ fontSize:10, color:'#8899BB', marginTop:2 }}>Ver clientes →</div>}
                  </div>
                ))}
              </div>

              {/* Oportunidades detectadas */}
              <div style={{ background:'#111827', border:'1px solid rgba(93,224,230,.1)', borderRadius:12, padding:'16px 18px', marginBottom:14 }}>
                <div style={{ fontSize:13, fontWeight:700, color:'#5DE0E6', marginBottom:12 }}>⚡ Oportunidades detectadas</div>
                {allOpps.length === 0 ? (
                  <div style={{ fontSize:12, color:'#22C55E', textAlign:'center' as 'center', padding:20 }}>
                    ✅ No hay oportunidades críticas pendientes
                  </div>
                ) : allOpps.map((o, i) => (
                  <div key={i} style={{ display:'grid', gridTemplateColumns:'32px 1fr auto auto', gap:10, alignItems:'center', padding:'8px 0', borderBottom:'1px solid rgba(93,224,230,.05)' }}>
                    <div style={{ fontSize:20, textAlign:'center' as 'center' }}>{o.icon}</div>
                    <div>
                      <div style={{ fontSize:12, fontWeight:600, color:'#F0F4FF' }}>{o.customer.full_name || o.customer.first_name}</div>
                      <div style={{ fontSize:10, color: o.color, marginTop:1 }}>{o.label}</div>
                    </div>
                    <div style={{ fontSize:11, color:'#8899BB' }}>{o.action}</div>
                    <div style={{ display:'flex', gap:4 }}>
                      {o.customer.phone && (
                        <a href={`https://wa.me/${(o.customer.whatsapp || o.customer.phone).replace(/\D/g,'')}`} target="_blank" rel="noreferrer"
                          style={{ border:'none', borderRadius:6, cursor:'pointer', fontFamily:'Montserrat,sans-serif', fontWeight:700, padding:'4px 8px', fontSize:10, background:'rgba(34,197,94,.1)', color:'#22C55E', textDecoration:'none' }}>
                          💬
                        </a>
                      )}
                      <button onClick={() => openProfile(o.customer)}
                        style={{ border:'none', borderRadius:6, cursor:'pointer', fontFamily:'Montserrat,sans-serif', fontWeight:700, padding:'4px 8px', fontSize:10, background:'rgba(93,224,230,.08)', color:'#5DE0E6' }}>
                        Ver →
                      </button>
                    </div>
                  </div>
              ...
              <div style={{ background:'#111827', border:'1px solid rgba(93,224,230,.1)', borderRadius:12, padding:'16px 18px' }}>
                ...
                {[...customers].filter(c => c.total_purchases > 0)
                  .sort((a,b) => calcScore(b) - calcScore(a)).slice(0, 8).map((c, i) => {
                  const score = calcScore(c)
                  return (
                    <div key={c.id} onClick={() => openProfile(c)}
                      style={{ display:'grid', gridTemplateColumns:'24px 1fr 80px 60px', gap:10, alignItems:'center', padding:'7px 0', borderBottom:'1px solid rgba(93,224,230,.04)', cursor:'pointer' }}
                      onMouseEnter={e => (e.currentTarget as HTMLDivElement).style.background = 'rgba(0,74,173,.06)'}
                      onMouseLeave={e => (e.currentTarget as HTMLDivElement).style.background = 'transparent'}
                    >
                      <div style={{ fontSize:12, fontWeight:800, color:'rgba(93,224,230,.4)', textAlign:'center' as 'center' }}>{i+1}</div>
                      <div>
                      ...
                      <div>
                        ...
                        <div style={{ fontSize:9, color:'#8899BB', textAlign:'right' as 'right' }}>{score}/100</div>
                      </div>
                      <div style={{ fontSize:12, fontWeight:800, color: scoreColor(score), textAlign:'right' as 'right' }}>
                        {score}
                      </div>
                    </div>
                  )
