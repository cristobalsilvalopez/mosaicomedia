...
export default function InventarioPage() {
  ...
  useEffect(() => {
    async function init() {
      ...
      await loadInventory(ctx.companyId)
      setLoading(false)
    }
    init()
  }, [])
  ...
  async function readInvoiceWithAI() {
    ...
    try {
      ...
      const response = await fetch('/api/ai/invoice', {
        ...
        body: JSON.stringify({ base64, mediaType: invoiceFile.type }),
      })

      if (!response.ok) throw new Error('Error del servidor')

      const result = await response.json()
      setInvoiceResult(result.products || [])
    } catch (err) {
    ...
    <div style={S.page}>
      ...
      <div style={S.body}>
        ...
        <div style={{ background:'#111827', border:'1px solid rgba(93,224,230,.1)', borderRadius:12, overflow:'hidden' }}>
          <div style={{ display:'grid', gridTemplateColumns:'1fr 100px 90px 90px 70px 70px 100px 140px', gap:8, padding:'8px 16px', background:'#0D1525', borderBottom:'1px solid rgba(93,224,230,.08)', fontSize:9, fontWeight:700, color:'#8899BB', textTransform:'uppercase' as const, letterSpacing:'.5px' }}>
            <span>Producto</span>
            ...
            return (
              ...
              >
                <div style={{ minWidth:0 }}>
                  ...
                  <div style={{ fontSize:10, color:'#8899BB', marginTop:2, display:'flex', gap:5, alignItems:'center', flexWrap:'wrap' as const }}>
