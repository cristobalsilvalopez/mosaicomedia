...
interface Customer {
  ...
  segment: string
  // Campos de inteligencia (calculados en DB)
  customer_score?: number; churn_risk?: number; repurchase_prob?: number
  trend?: string; lifetime_value?: number; purchase_frequency?: number
}

// ============================================================
// INTELIGENCIA — calculado en cliente desde datos reales
// ============================================================

function calcScore(c: Customer): number {
  // Score 0-100 basado en recencia, frecuencia y valor (RFM)
  let score = 0
  // Frecuencia (hasta 35 pts)
  if (c.total_purchases >= 10) score += 35
  else if (c.total_purchases >= 5)  score += 25
  else if (c.total_purchases >= 3)  score += 15
  else if (c.total_purchases >= 1)  score += 8
  // Recencia (hasta 35 pts)
  const days = c.days_since_purchase ?? 999
  if (days <= 7)        score += 35
  else if (days <= 30)  score += 28
  else if (days <= 60)  score += 18
  else if (days <= 90)  score += 10
  else if (days <= 180) score += 4
  // Valor (hasta 30 pts)
  if (c.total_spent >= 1000000)     score += 30
  else if (c.total_spent >= 500000) score += 22
  else if (c.total_spent >= 200000) score += 15
  else if (c.total_spent >= 50000)  score += 8
  else if (c.total_spent >= 10000)  score += 3
  return Math.min(100, score)
}

type Opportunity = { type: string; label: string; action: string; color: string; icon: string }

function detectOpportunities(c: Customer): Opportunity[] {
  const ops: Opportunity[] = []
  const days = c.days_since_purchase ?? 999
  const score = calcScore(c)

  if (c.total_purchases === 0) {
    ops.push({ type:'welcome', label:'Cliente sin compras', action:'Enviar bienvenida', color:'#22C55E', icon:'👋' })
  }
  if (days > 90 && c.total_purchases >= 3) {
    ops.push({ type:'reactivate', label:`Sin comprar hace ${days}d`, action:'Ofrecer promoción', color:'#EF4444', icon:'🔥' })
  } else if (days > 30 && days <= 90 && c.total_purchases >= 2) {
    ops.push({ type:'followup', label:'Baja frecuencia reciente', action:'Contactar por WhatsApp', color:'#F59E0B', icon:'📲' })
  }
  if (c.total_spent >= 300000 && c.tier === 'standard') {
    ops.push({ type:'upgrade', label:'Listo para nivel Gold', action:'Asignar beneficios VIP', color:'#C19E4D', icon:'⭐' })
  }
  if (c.total_purchases === 1 && days <= 30) {
    ops.push({ type:'second_sale', label:'Primera compra reciente', action:'Impulsar segunda compra', color:'#5DE0E6', icon:'🎯' })
  }
  if (score >= 70 && c.segment !== 'vip') {
    ops.push({ type:'vip_potential', label:'Alto potencial VIP', action:'Ofrecer programa exclusivo', color:'#A78BFA', icon:'💎' })
  }
  return ops
}
...
export default function CRMPage() {
  ...
  // Tab activo en lista (list | intelligence | campaigns)
  const [activeTab, setActiveTab] = useState<'list'|'intelligence'|'campaigns'>('list')
  ...
  const [campSegment, setCampSegment]         = useState('at_risk')
  ...
  async function loadIntelligence(companyId?: string) {
    const cId = companyId || company?.id
    if (!cId) return
    const { data } = await supabase.rpc('get_crm_intelligence', { p_company_id: cId })
    setIntel(data)
    if (data?.pending_msgs > 0) {
      const { data: msgs } = await supabase.rpc('get_pending_messages', {
  ...
  async function loadTemplates() {
    const { data } = await supabase
      ...
      .select('id, name, content, variables, channel')
      .eq('is_active', true)
      .order('name')
    setTemplates(data || [])
  }

  async function loadCampaigns(companyId?: string) {
    const cId = companyId || company?.id
    if (!cId) return
    const { data } = await supabase
      .from('crm_campaigns')
      .select('id, name, segment, status, target_count, sent_count, created_at, completed_at')
      .eq('company_id', cId)
  ...
  async function launchCampaign() {
    if (!campName || !campSegment || !campTemplateId) return
    setLaunching(true); setLaunchResult(null)
    const { data, error } = await supabase.rpc('launch_campaign', {
      p_company_id:  company.id,
      p_name:        campName,
      p_segment:     campSegment,
      p_template_id: campTemplateId,
      p_channel:     'whatsapp',
    })
    setLaunching(false)
    if (error) { setLaunchResult({ error: error.message }); return }
    setLaunchResult(data)
    await Promise.all([loadCampaigns(), loadIntelligence()])
    if (data?.queued > 0) {
      const { data: msgs } = await supabase.rpc('get_pending_messages', {
        p_company_id: company.id, p_limit: 50
      })
      setPendingMsgs(msgs || [])
    }
  }

  // Abrir WhatsApp nativo con el mensaje prellenado
  async function sendViaWhatsApp(msg: any) {
    ...
    await supabase.rpc('update_message_status', {
      p_message_id: msg.id, p_status: 'sent'
    })
    setPendingMsgs(prev => prev.filter(m => m.id !== msg.id))
  ...
  useEffect(() => {
    async function init() {
      ...
      await Promise.all([
        loadCustomers(ctx.companyId),
        loadSummary(ctx.companyId),
        loadIntelligence(ctx.companyId),
        loadTemplates(),
        loadCampaigns(ctx.companyId),
      ])
      setLoading(false)
    }
    init()
            ...
            <div style={{ display:'grid', gridTemplateColumns:'340px 1fr', gap:16 }}>
              ...
              <div>
                ...
                <div style={{ ...S.card, textAlign:'center' as const }}>
                  ...
                  <div style={{ display:'flex', justifyContent:'center', gap:8, marginTop:10, flexWrap:'wrap' as const }}>
                  ...
                  {(c.tags || []).length > 0 && (
                    <div style={{ display:'flex', flexWrap:'wrap' as const, gap:4, justifyContent:'center', marginTop:10 }}>
                      {(c.tags || []).map((t: string) => (
                ...
                <div style={S.card}>
                  ...
                  {[
                    ...
                    <div key={l} style={{ display:'flex', justifyContent:'space-between', padding:'4px 0', fontSize:12, borderBottom:'1px solid rgba(93,224,230,.05)' }}>
                      ...
                      <span style={{ fontWeight:600, maxWidth:160, textAlign:'right' as const, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' as const }}>{v}</span>
                    </div>
                  ))}
                ...
                <div style={S.card}>
                  ...
                  {(() => {
                    const score = calcScore(c)
                    return (
                      <div style={{ marginTop:10, padding:'10px', background:'#0D1525', borderRadius:8 }}>
                        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:6 }}>
                          <div style={{ fontSize:10, color:'#8899BB' }}>Score del cliente</div>
                          <div style={{ fontSize:18, fontWeight:800, color: scoreColor(score) }}>{score}/100</div>
                        </div>
                        <div style={{ height:8, background:'rgba(93,224,230,.08)', borderRadius:4, overflow:'hidden' }}>
                          <div style={{ height:'100%', width:`${score}%`, background: `linear-gradient(90deg, ${scoreColor(score)}88, ${scoreColor(score)})`, borderRadius:4, transition:'width .5s' }} />
                        </div>
                        <div style={{ fontSize:9, color:'#8899BB', marginTop:4 }}>
                          {score >= 70 ? 'Cliente de alto valor' : score >= 40 ? 'Cliente regular activo' : 'Necesita atención'}
                        </div>
                      </div>
                    )
                  })()}
                ...
                <div style={S.card}>
                  ...
                  <div style={{ display:'flex', flexDirection:'column' as const, gap:6 }}>
                    {selected.phone && (
                      <a href={`https://wa.me/${(selected.whatsapp || selected.phone).replace(/\D/g,'')}`} target="_blank" rel="noreferrer"
                        style={{ ...S.btn, padding:'9px 12px', fontSize:12, background:'rgba(34,197,94,.1)', border:'1px solid rgba(34,197,94,.2)', color:'#22C55E', textAlign:'center' as const, textDecoration:'none', display:'block' }}>
                        💬 Enviar WhatsApp
                      </a>
                    )}
                    {selected.email && (
                      <a href={`mailto:${selected.email}`}
                        style={{ ...S.btn, padding:'9px 12px', fontSize:12, background:'rgba(93,224,230,.08)', border:'1px solid rgba(93,224,230,.2)', color:'#5DE0E6', textAlign:'center' as const, textDecoration:'none', display:'block' }}>
                        📧 Enviar email
                      </a>
                    )}
              ...
              <div>
                ...
                {(() => {
                  const opps = detectOpportunities(c)
                  if (!opps.length) return null
                  return (
                    <div style={{ background:'#111827', border:'1px solid rgba(239,68,68,.2)', borderRadius:12, padding:'14px 16px', marginBottom:14 }}>
                      <div style={{ fontSize:12, fontWeight:700, color:'#EF4444', marginBottom:10 }}>⚡ Oportunidades detectadas</div>
                      {opps.map((o, i) => (
                        <div key={i} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'7px 0', borderBottom:'1px solid rgba(239,68,68,.06)', fontSize:12 }}>
                          <div>
                            <span style={{ marginRight:8 }}>{o.icon}</span>
                            <span style={{ color: o.color, fontWeight:600 }}>{o.label}</span>
                          </div>
                          <div style={{ display:'flex', gap:6, alignItems:'center' }}>
                            <span style={{ fontSize:10, color:'#8899BB' }}>{o.action}</span>
                            {c.phone && (
                              <a href={`https://wa.me/${(c.whatsapp || c.phone).replace(/\D/g,'')}`} target="_blank" rel="noreferrer"
                                style={{ border:'none', borderRadius:6, cursor:'pointer', fontFamily:'Montserrat,sans-serif', fontWeight:700, padding:'3px 8px', fontSize:11, background:'rgba(34,197,94,.1)', color:'#22C55E', textDecoration:'none' }}>
                                💬
                              </a>
                            )}
                          </div>
                ...
                <div style={S.card}>
                  ...
                  {!profile?.sales?.length ? (
                    <div style={{ fontSize:12, color:'#8899BB', padding:'10px 0', textAlign:'center' as const }}>Sin compras registradas</div>
                ...
                <div style={S.card}>
                  ...
                  <div style={{ background:'#0D1525', borderRadius:10, padding:'12px', marginBottom:12 }}>
                    ...
                    <textarea value={noteText} onChange={e => setNoteText(e.target.value)}
                      ...
                      rows={2} style={{ ...S.input, resize:'vertical' as const, marginBottom:8 }}
                  ...
                  {!profile?.notes?.length ? (
                    <div style={{ fontSize:12, color:'#8899BB', textAlign:'center' as const, padding:'8px 0' }}>Sin notas</div>
  ...
  return (
    <div style={S.page}>
      ...
      <div style={S.body}>
        ...
        {/* Tabs: Lista | Inteligencia | Campañas */}
        <div style={{ display:'flex', gap:4, background:'#111827', border:'1px solid rgba(93,224,230,.1)', borderRadius:10, padding:4, marginBottom:14, width:'fit-content' }}>
          {([
            ...
            <button key={t} onClick={() => setActiveTab(t)}
              style={{ border:'none', borderRadius:7, cursor:'pointer', fontFamily:'Montserrat,sans-serif', fontWeight:700, padding:'6px 16px', fontSize:12, background: activeTab === t ? 'linear-gradient(90deg,#004AAD,#5DE0E6)' : 'transparent', color: activeTab === t ? '#fff' : '#8899BB', position:'relative' }}>
              {l}
              {t === 'campaigns' && intel?.pending_msgs > 0 && (
                <span style={{ position:'absolute', top:2, right:4, fontSize:9, background:'#EF4444', color:'#fff', borderRadius:10, padding:'1px 4px', fontWeight:800 }}>
                  {intel.pending_msgs}
                </span>
              )}
            </button>
        ...
        {/* ============ PANEL DE INTELIGENCIA (DB-powered) ============ */}
        {activeTab === 'intelligence' && (
          <div>
            {/* KPIs de inteligencia */}
            <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:10, marginBottom:16 }}>
              {[
                { label:'📊 Score promedio', value: intel ? `${intel.avg_score}/100` : '—', color:'#5DE0E6', bg:'rgba(93,224,230,.06)' },
                { label:'🔥 Alto riesgo fuga', value: intel?.high_churn || 0, color:'#EF4444', bg:'rgba(239,68,68,.06)', action: () => { setFilterSegment('at_risk'); setActiveTab('list') } },
                { label:'💎 Potencial VIP',    value: intel?.vip_ready || 0,  color:'#C19E4D', bg:'rgba(193,158,77,.06)' },
                { label:'📲 Msgs. en cola',    value: intel?.pending_msgs || 0, color:'#22C55E', bg:'rgba(34,197,94,.06)', action: () => setActiveTab('campaigns') },
              ].map(k => (
                <div key={k.label} onClick={(k as any).action}
                  style={{ background: k.bg, border:`1px solid ${k.color}25`, borderRadius:10, padding:'12px 14px', cursor: (k as any).action ? 'pointer' : 'default' }}>
                  <div style={{ fontSize:10, color: k.color, fontWeight:600 }}>{k.label}</div>
                  <div style={{ fontSize:24, fontWeight:800, color:'#F0F4FF', marginTop:4 }}>{k.value}</div>
                  {(k as any).action && <div style={{ fontSize:10, color:'#8899BB', marginTop:2 }}>Ver →</div>}
                </div>
              ))}
            </div>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:14, marginBottom:14 }}>
              ...
              <div style={{ background:'#111827', border:'1px solid rgba(239,68,68,.2)', borderRadius:12, padding:'14px 16px' }}>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:10 }}>
                  <div style={{ fontSize:13, fontWeight:700, color:'#EF4444' }}>🔥 En riesgo de fuga</div>
                  <button onClick={() => { setCampSegment('at_risk'); setShowCampaign(true) }}
                    style={{ border:'none', borderRadius:6, cursor:'pointer', fontFamily:'Montserrat,sans-serif', fontWeight:700, padding:'3px 10px', fontSize:10, background:'rgba(239,68,68,.12)', color:'#EF4444' }}>
                    📢 Campaña
                  </button>
                </div>
                {!intel?.at_risk_list?.length ? (
                  <div style={{ fontSize:12, color:'#22C55E', textAlign:'center' as const, padding:12 }}>✅ Sin clientes en riesgo</div>
                ) : intel.at_risk_list.map((c: any) => (
                  <div key={c.id} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'6px 0', borderBottom:'1px solid rgba(239,68,68,.06)', fontSize:12 }}>
                    <div>
                      <div style={{ fontWeight:600 }}>{c.full_name}</div>
                      <div style={{ fontSize:10, color:'#EF4444' }}>Hace {c.days_since}d · Riesgo {Math.round(c.churn_risk)}%</div>
                    </div>
                    <div style={{ display:'flex', gap:4 }}>
                      ...
                      <button onClick={() => { const found = customers.find(cu => cu.id === c.id); if (found) openProfile(found) }}
                        style={{ border:'none', borderRadius:6, cursor:'pointer', fontFamily:'Montserrat,sans-serif', fontWeight:700, padding:'3px 8px', fontSize:11, background:'rgba(93,224,230,.08)', color:'#5DE0E6' }}>
                        Ver
                      </button>
                    </div>
                  </div>
              ...
              {/* Potencial VIP */}
              <div style={{ background:'#111827', border:'1px solid rgba(193,158,77,.2)', borderRadius:12, padding:'14px 16px' }}>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:10 }}>
                  <div style={{ fontSize:13, fontWeight:700, color:'#C19E4D' }}>💎 Potencial VIP</div>
                  <button onClick={() => { setCampSegment('vip'); setShowCampaign(true) }}
                    style={{ border:'none', borderRadius:6, cursor:'pointer', fontFamily:'Montserrat,sans-serif', fontWeight:700, padding:'3px 10px', fontSize:10, background:'rgba(193,158,77,.12)', color:'#C19E4D' }}>
                    📢 Campaña
                  </button>
                </div>
                {!intel?.vip_ready_list?.length ? (
                  <div style={{ fontSize:12, color:'#8899BB', textAlign:'center' as const, padding:12 }}>Sin potenciales VIP detectados</div>
                ) : intel.vip_ready_list.map((c: any) => (
                  <div key={c.id} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'6px 0', borderBottom:'1px solid rgba(193,158,77,.06)', fontSize:12 }}>
                    <div>
                      <div style={{ fontWeight:600 }}>{c.full_name}</div>
                      <div style={{ fontSize:10, color:'#C19E4D' }}>Score {c.customer_score} · {fmt(c.total_spent)}</div>
                    </div>
                    <div style={{ display:'flex', gap:4 }}>
                      {c.phone && (
                        <a href={`https://wa.me/${c.phone.replace(/\D/g,'')}`} target="_blank" rel="noreferrer"
            ...
            {/* Top clientes por score (DB) */}
            <div style={{ background:'#111827', border:'1px solid rgba(93,224,230,.1)', borderRadius:12, padding:'14px 16px' }}>
              <div style={{ fontSize:13, fontWeight:700, color:'#5DE0E6', marginBottom:12 }}>🏆 Top clientes por score</div>
              {[...customers].filter(c => c.total_purchases > 0)
                .sort((a,b) => (b.customer_score || calcScore(b)) - (a.customer_score || calcScore(a)))
                .slice(0, 8).map((c, i) => {
                const score = c.customer_score ?? calcScore(c)
                return (
                  <div key={c.id} onClick={() => openProfile(c)}
                    style={{ display:'grid', gridTemplateColumns:'24px 1fr 100px 60px', gap:10, alignItems:'center', padding:'7px 0', borderBottom:'1px solid rgba(93,224,230,.04)', cursor:'pointer' }}
                    onMouseEnter={e => (e.currentTarget as HTMLDivElement).style.background = 'rgba(0,74,173,.06)'}
                    onMouseLeave={e => (e.currentTarget as HTMLDivElement).style.background = 'transparent'}
                  >
                    <div style={{ fontSize:12, fontWeight:800, color:'rgba(93,224,230,.4)', textAlign:'center' as const }}>{i+1}</div>
                    <div>
                      <div style={{ fontSize:12, fontWeight:600 }}>{c.full_name || c.first_name}</div>
                      <div style={{ fontSize:10, color:'#8899BB' }}>{c.total_purchases} compras · {fmt(c.total_spent)}</div>
                    </div>
                    <div>
                      <div style={{ height:6, background:'rgba(93,224,230,.08)', borderRadius:3, overflow:'hidden', marginBottom:2 }}>
                        <div style={{ height:'100%', width:`${score}%`, background: scoreColor(score), borderRadius:3 }} />
                      </div>
                      <div style={{ fontSize:9, color:'#8899BB', textAlign:'right' as const }}>{score}/100</div>
                    </div>
                    <div style={{ fontSize:13, fontWeight:800, color: scoreColor(score), textAlign:'right' as const }}>{score}</div>
                  </div>
                )
              })}
            </div>
          ...
          <div>
            ...
            {showCampaign && (
              <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,.8)', zIndex:500, display:'flex', alignItems:'center', justifyContent:'center' }}>
                <div style={{ background:'#111827', border:'1px solid rgba(93,224,230,.25)', borderRadius:14, padding:'22px 24px', width:440, color:'#F0F4FF' }}>
                  <div style={{ fontSize:14, fontWeight:700, marginBottom:4 }}>📢 Nueva campaña WhatsApp</div>
                  <div style={{ fontSize:11, color:'#8899BB', marginBottom:16 }}>Los mensajes se encolan y tú los envías uno a uno con WhatsApp abierto.</div>

                  {launchResult ? (
                    <div>
                      {launchResult.error ? (
                        <div style={{ background:'rgba(239,68,68,.1)', border:'1px solid rgba(239,68,68,.2)', borderRadius:8, padding:'12px', fontSize:13, color:'#EF4444', marginBottom:14 }}>
                          ❌ {launchResult.error}
                        </div>
                      ) : (
                        <div style={{ background:'rgba(34,197,94,.08)', border:'1px solid rgba(34,197,94,.2)', borderRadius:8, padding:'12px', fontSize:13, color:'#22C55E', marginBottom:14, textAlign:'center' as const }}>
                          <div style={{ fontSize:28 }}>✅</div>
                          <div style={{ fontWeight:700, marginTop:4 }}>{launchResult.queued} mensajes encolados</div>
                          <div style={{ fontSize:11, color:'#8899BB', marginTop:4 }}>Ve a la pestaña Campañas para enviarlos</div>
                        </div>
                      )}
                      <button onClick={() => { setShowCampaign(false); setLaunchResult(null); setCampName('') }}
                        style={{ border:'none', borderRadius:8, cursor:'pointer', fontFamily:'Montserrat,sans-serif', fontWeight:700, width:'100%', padding:11, fontSize:13, background:'rgba(93,224,230,.1)', color:'#5DE0E6' }}>
                        Cerrar
                      </button>
                    </div>
                  ) : (
                    <>
                      <label style={{ fontSize:11, fontWeight:600, color:'#8899BB', marginBottom:4, display:'block' }}>Nombre de la campaña</label>
                      <input value={campName} onChange={e => setCampName(e.target.value)}
                        placeholder="Ej: Reactivación abril 2026"
                        style={{ background:'#1A2540', border:'1px solid rgba(93,224,230,.2)', borderRadius:7, padding:'8px 10px', fontSize:13, color:'#F0F4FF', outline:'none', fontFamily:'Montserrat,sans-serif', width:'100%', boxSizing:'border-box' as const, marginBottom:12 }} />

                      <label style={{ fontSize:11, fontWeight:600, color:'#8899BB', marginBottom:4, display:'block' }}>Segmento objetivo</label>
                      <select value={campSegment} onChange={e => setCampSegment(e.target.value)}
                        style={{ background:'#1A2540', border:'1px solid rgba(93,224,230,.2)', borderRadius:7, padding:'8px 10px', fontSize:13, color:'#F0F4FF', outline:'none', fontFamily:'Montserrat,sans-serif', width:'100%', boxSizing:'border-box' as const, marginBottom:12 }}>
                        <option value="at_risk">⚠️ En riesgo (sin comprar 90+ días)</option>
                        <option value="dormant">😴 Dormidos (60+ días)</option>
                      ...
                      <label style={{ fontSize:11, fontWeight:600, color:'#8899BB', marginBottom:4, display:'block' }}>Plantilla de mensaje</label>
                      <select value={campTemplateId} onChange={e => setCampTemplateId(e.target.value)}
                        style={{ background:'#1A2540', border:'1px solid rgba(93,224,230,.2)', borderRadius:7, padding:'8px 10px', fontSize:13, color:'#F0F4FF', outline:'none', fontFamily:'Montserrat,sans-serif', width:'100%', boxSizing:'border-box' as const, marginBottom: campTemplateId ? 10 : 16 }}>
                        <option value="">Seleccionar plantilla...</option>
                        {templates.map(t => <option key={t.id} value={t.id}>{t.name}</option>)}
                      </select>

                      {campTemplateId && (
                        <div style={{ background:'#0D1525', borderRadius:8, padding:'10px 12px', fontSize:11, color:'#8899BB', marginBottom:14, fontStyle:'italic' }}>
                          {templates.find(t => t.id === campTemplateId)?.content}
                        </div>
                      )}

                      <div style={{ display:'flex', gap:8 }}>
                        <button onClick={() => { setShowCampaign(false); setLaunchResult(null) }}
                          style={{ border:'1px solid rgba(255,255,255,.1)', borderRadius:8, cursor:'pointer', fontFamily:'Montserrat,sans-serif', fontWeight:700, flex:1, padding:11, fontSize:12, background:'rgba(255,255,255,.05)', color:'#8899BB' }}>
                          Cancelar
                        </button>
                        <button onClick={launchCampaign} disabled={launching || !campName || !campSegment || !campTemplateId}
                          style={{ border:'none', borderRadius:8, cursor:'pointer', fontFamily:'Montserrat,sans-serif', fontWeight:700, flex:2, padding:11, fontSize:13, background: !campName || !campTemplateId ? 'rgba(0,74,173,.2)' : 'linear-gradient(90deg,#004AAD,#5DE0E6)', color:'#fff', opacity: !campName || !campTemplateId ? .5 : 1 }}>
                          {launching ? '⏳ Creando...' : '🚀 Lanzar campaña'}
                        </button>
                      </div>
                    </>
                  )}
            ...
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:14 }}>
              <div>
                <div style={{ fontSize:14, fontWeight:700 }}>📢 Campañas WhatsApp</div>
                <div style={{ fontSize:11, color:'#8899BB', marginTop:2 }}>Mensajes encolados listos para enviar: <strong style={{ color:'#5DE0E6' }}>{pendingMsgs.length}</strong></div>
              </div>
              <button onClick={() => { setLaunchResult(null); setShowCampaign(true) }}
                style={{ border:'none', borderRadius:8, cursor:'pointer', fontFamily:'Montserrat,sans-serif', fontWeight:700, background:'linear-gradient(90deg,#004AAD,#5DE0E6)', color:'#fff', padding:'7px 16px', fontSize:12 }}>
                + Nueva campaña
              </button>
            </div>
            ...
            {pendingMsgs.length > 0 && (
              <div style={{ background:'#111827', border:'1px solid rgba(34,197,94,.2)', borderRadius:12, padding:'14px 16px', marginBottom:14 }}>
                <div style={{ fontSize:12, fontWeight:700, color:'#22C55E', marginBottom:10 }}>
                  📲 {pendingMsgs.length} mensajes listos para enviar
                </div>
                <div style={{ fontSize:11, color:'#8899BB', marginBottom:12 }}>
                  Haz clic en 💬 para abrir WhatsApp con el mensaje prellenado. Se marca como enviado automáticamente.
                </div>
                <div style={{ maxHeight:280, overflowY:'auto' as const }}>
                  {pendingMsgs.map((m, i) => (
                    <div key={m.id} style={{ display:'grid', gridTemplateColumns:'1fr auto', gap:10, alignItems:'center', padding:'8px 0', borderBottom:'1px solid rgba(34,197,94,.06)', fontSize:12 }}>
                      <div>
                        <div style={{ fontWeight:600 }}>{m.first_name} {m.last_name}</div>
                        <div style={{ fontSize:10, color:'#8899BB', marginTop:1 }}>{m.to_phone} · {m.message.slice(0, 60)}...</div>
                      </div>
                      <button onClick={() => sendViaWhatsApp(m)}
                        style={{ border:'none', borderRadius:8, cursor:'pointer', fontFamily:'Montserrat,sans-serif', fontWeight:700, padding:'6px 14px', fontSize:12, background:'rgba(34,197,94,.15)', color:'#22C55E' }}>
                        💬 Enviar
                      </button>
                    </div>
                  ))}
            ...
            <div style={{ background:'#111827', border:'1px solid rgba(93,224,230,.1)', borderRadius:12, overflow:'hidden' }}>
              <div style={{ padding:'12px 16px', background:'#0D1525', borderBottom:'1px solid rgba(93,224,230,.08)', fontSize:11, fontWeight:700, color:'#8899BB', textTransform:'uppercase' as const, letterSpacing:'.5px' }}>
                Historial de campañas
              </div>
              {campaigns.length === 0 ? (
                <div style={{ textAlign:'center' as const, padding:40, color:'#8899BB', fontSize:12 }}>
                  Sin campañas creadas. Crea tu primera campaña arriba.
                </div>
              ) : campaigns.map((c, idx) => (
                <div key={c.id} style={{ display:'grid', gridTemplateColumns:'1fr 120px 100px 100px', gap:8, padding:'10px 16px', borderBottom:'1px solid rgba(93,224,230,.04)', background: idx % 2 === 0 ? 'transparent' : 'rgba(255,255,255,.01)', fontSize:12 }}>
                  <div>
                    <div style={{ fontWeight:600 }}>{c.name}</div>
                    <div style={{ fontSize:10, color:'#8899BB', marginTop:1 }}>
                      {({ at_risk:'⚠️ En riesgo', dormant:'😴 Dormidos', vip:'⭐ VIP', new:'🆕 Nuevos', all:'👥 Todos' } as any)[c.segment] || c.segment}
                      {' · '}{new Date(c.created_at).toLocaleDateString('es-CL')}
                    </div>
                  </div>
                  <div style={{ textAlign:'center' as const, alignSelf:'center', fontSize:11, color:'#8899BB' }}>
                    {c.sent_count}/{c.target_count} enviados
                  </div>
                  <div style={{ textAlign:'center' as const, alignSelf:'center' }}>
                    <span style={{ fontSize:10, fontWeight:700, padding:'2px 8px', borderRadius:20,
                      background: c.status === 'completed' ? 'rgba(34,197,94,.1)' : c.status === 'running' ? 'rgba(93,224,230,.1)' : 'rgba(255,255,255,.05)',
                      color:      c.status === 'completed' ? '#22C55E' : c.status === 'running' ? '#5DE0E6' : '#8899BB' }}>
                      {c.status === 'completed' ? 'Completada' : c.status === 'running' ? 'En curso' : c.status}
                    </span>
                  </div>
                  <div style={{ textAlign:'right' as const, alignSelf:'center', fontSize:10, color:'#8899BB' }}>
                    {c.target_count > 0 ? `${Math.round(c.sent_count / c.target_count * 100)}%` : '—'}
                  </div>
                </div>
              ))}
            </div>
        ...
        {/* ============ LISTA DE CLIENTES ============ */}
        {activeTab === 'list' && <div style={{ background:'#111827', border:'1px solid rgba(93,224,230,.1)', borderRadius:12, overflow:'hidden' }}>
          {/* Header */}
          <div style={{ display:'grid', gridTemplateColumns:'1fr 120px 130px 120px 110px 100px', gap:8, padding:'8px 16px', background:'#0D1525', borderBottom:'1px solid rgba(93,224,230,.08)', fontSize:9, fontWeight:700, color:'#8899BB', textTransform:'uppercase' as const, letterSpacing:'.5px' }}>
            <span>Cliente</span>
            <span style={{ textAlign:'right' as const }}>Total gastado</span>
            <span style={{ textAlign:'center' as const }}>Última compra</span>
            <span style={{ textAlign:'center' as const }}>Ticket prom.</span>
            <span style={{ textAlign:'center' as const }}>Segmento</span>
            <span style={{ textAlign:'center' as const }}>Nivel</span>
          </div>

          {filtered.length === 0 ? (
            <div style={{ textAlign:'center' as const, padding:50, color:'#8899BB' }}>
            ...
            return (
              ...
              >
                <div style={{ minWidth:0 }}>
                  <div style={{ fontSize:13, fontWeight:700, whiteSpace:'nowrap' as const, overflow:'hidden', textOverflow:'ellipsis' }}>
                ...
                <div style={{ fontSize:13, fontWeight:800, color:'#5DE0E6', textAlign:'right' as const, alignSelf:'center' }}>
                  {c.total_purchases > 0 ? fmt(c.total_spent) : <span style={{ color:'#6B7280', fontSize:11 }}>Sin compras</span>}
                </div>
                <div style={{ textAlign:'center' as const, fontSize:11, alignSelf:'center' }}>
                  {c.last_purchase_at ? (
                    <>
                ...
                <div style={{ textAlign:'center' as const, fontSize:12, fontWeight:700, alignSelf:'center' }}>
