...
export default function POSPage() {
  ...
  useEffect(() => {
    async function init() {
      ...
      await Promise.all([
        ...
        loadCashSession(userData.company_id),
      ])
      setLoading(false)
    }
    init()
  ...
  async function completeSale() {
    ...
    await loadCashSession(company.id)
  }
  ...
  if (!cashSessionLoading && !cashSession) return (
    <div style={{ minHeight:'100vh', background:'#0A1628', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', fontFamily:'Montserrat,sans-serif', color:'#F0F4FF', gap:16, padding:20 }}>
      <div style={{ width:60, height:60, borderRadius:15, background:'linear-gradient(135deg,#004AAD,#5DE0E6)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:28 }}>🏪</div>
      ...
      <div style={{ fontSize:13, color:'#8899BB', textAlign:'center', maxWidth:340, lineHeight:1.7 }}>
        Debes abrir la caja antes de registrar ventas.<br/>Ve al módulo de caja para iniciar una sesión.
      </div>
      <div style={{ display:'flex', gap:10, marginTop:4 }}>
        <button
          onClick={() => router.push('/caja')}
          style={{ border:'none', borderRadius:9, cursor:'pointer', fontFamily:'Montserrat,sans-serif', fontWeight:700, padding:'12px 24px', fontSize:13, background:'linear-gradient(90deg,#004AAD,#5DE0E6)', color:'#fff' }}
        >
          🏪 Abrir caja
        </button>
        <button
          onClick={() => router.push('/dashboard')}
          style={{ borderRadius:9, cursor:'pointer', fontFamily:'Montserrat,sans-serif', fontWeight:700, padding:'12px 24px', fontSize:13, background:'rgba(255,255,255,.05)', border:'1px solid rgba(93,224,230,.2)', color:'#8899BB' } as React.CSSProperties}
        >
          ← Dashboard
        </button>
      </div>
