...
export default function CajaPage() {
  ...
  async function handleOpen() {
    ...
    // Verificar PIN si el usuario tiene PIN habilitado
    if (pin.length >= 4) {
    ...
    <div style={st.page}>
      ...
      <div style={st.body}>
        <div style={st.card}>
          ...
          {/* PIN */}
          <label style={st.label}>PIN del cajero (4-6 dígitos)</label>
          <div style={{ position:'relative', marginBottom:14 }}>
            <input
              ...
              maxLength={6}
              style={{ ...st.input, letterSpacing:6, fontSize:18, fontWeight:700 }}
              autoFocus
            />
            {pin.length >= 4 && (
              <div style={{ position:'absolute', right:12, top:'50%', transform:'translateY(-50%)', fontSize:10, color:'#22C55E', fontWeight:700 }}>
                {'●'.repeat(pin.length)}
              </div>
            )}
          </div>
          {pinError && <div style={{ background:'rgba(239,68,68,.1)', border:'1px solid rgba(239,68,68,.2)', borderRadius:6, padding:'6px 10px', fontSize:11, color:'#EF4444', marginBottom:10 }}>{pinError}</div>}
